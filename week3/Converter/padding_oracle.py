...
class PaddingOracle:
    def __init__(self, oracle, block_size=16, max_retries=DEFAULT_MAX_RETRIES,
                 retry_wait_seconds=DEFAULT_RETRY_WAIT_SECONDS, log_level=LOG_INFO):
        self.oracle = oracle
    ...
    def _call_oracle(self, cipher_hex):
        ...
        raise PaddingOracleException(
            'Oracle finally failed after {} retries!'.format(self.max_retries)) from last_exception

    def decrypt_block(self, iv_block, cipher_block, pre_known=[], detect_padding=False,
                      optimized_alphabet=printable_ascii_alphabet()):
        """
        ...
        if len(cipher_block) // 2 != self.block_size:
            raise PaddingOracleException(
                'Cipher size is not block size: {} != {}'.format(len(cipher_block) // 2, self.block_size))
        ...
        for i in range(index_start, self.block_size):
            ...
            for byte_index, byte_value in enumerate(current_alphabet):
                ...
                self.logger.log_byte_status(current_iv[index], byte_value, byte_index, len(current_alphabet), i,
                                            len(plain), cipher_hex)
    ...
    def decrypt(self, cipher, optimized_alphabet=printable_ascii_alphabet()):
        ...
        for i in range(0, len(cipher_blocks) - 1):
            ...
            plain_block, intermediate, padding = self.decrypt_block(iv, cipher_block, detect_padding=detect_padding,
                                                                    optimized_alphabet=optimized_alphabet)
    ...
    def craft(self, cipher, plain_old, plain_new):
        ...
        if len(plain_old) != len(plain_new):
            raise PaddingOracleException(
                'The plaintexts differ in size! old={} new={}'.format(len(plain_old), len(plain_new)))
        ...
        for i in range(len(cipher_blocks) - 1, 0, -1):
            ...
            if nothing_changed_yet:
                ...
                cipher_new_block = int_array_to_hex(
                    [(a ^ b ^ c) for a, b, c in zip(plain_old_block, plain_new_block, hex_to_int_array(iv))])
